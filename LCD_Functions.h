...
byte displayMap[LCD_WIDTH * LCD_HEIGHT / 8] = {
  ...
  0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, // (36,24)->(47,31)
  0xFF, 0xFF, 0xFF, 0x7F, 0x3F, 0x1F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // (48,24)->(59,31)
...
int getPixel(int x, int y) {
  ...
  return (displayMap[x + (y / 8) * LCD_WIDTH] >> (y % 8)) & 0x1;  
}
...
void setLetter(int* pixels, int xPos, int yPos, boolean bw) {
  ...
  for (int index = 0; index < size; index++) {
    ...
    if (pixels[index] == 1)
      setPixel(x + xPos, y + yPos, bw);
  }
}

void setPixels(int* pixels, int count, boolean bw) {
 
  for (int pixelPosition = 0; pixelPosition < count; pixelPosition++) {
    
    byte y = pixels[pixelPosition] / LCD_WIDTH;
    byte x = pixels[pixelPosition] - y * LCD_WIDTH;
    setPixel(x, y, bw);
  }
}
